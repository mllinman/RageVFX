...
export class SubscriptionManager {
  ...
  private tierBadgeClickHandler: (() => void) | null = null;

  // Cache node access map to avoid repeated object creation
  private static readonly nodeAccessMap: Record<string, NodeAccess> = {
    // Pro-only nodes
    'BloodSplatterNode': {
      id: 'blood-splatter',
      name: 'Blood Splatter',
  ...
  private getNodeAccess(nodeType: string): NodeAccess | null {
    return SubscriptionManager.nodeAccessMap[nodeType] || null;
  }
  ...
  private updateUI(): void {
    ...
    if (header) {
      ...
      if (this.tierBadgeClickHandler) {
        tierBadge.removeEventListener('click', this.tierBadgeClickHandler);
        this.tierBadgeClickHandler = null;
      }

      // Add click handler for upgrade (free tier only)
      if (this.currentTier === 'free') {
        tierBadge.style.cursor = 'pointer';
        this.tierBadgeClickHandler = () => {
          this.showUpgradeDialog();
        };
        tierBadge.addEventListener('click', this.tierBadgeClickHandler);
      } else {
        tierBadge.style.cursor = 'default';
      }
    }
  }
